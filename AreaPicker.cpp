...
vtkStandardNewMacro(AreaPicker);
...
//--------------------------------------------------------------------------
void AreaPicker::SetRenderer(vtkRenderer* renderer)
{
  this->Renderer = renderer;
}
//--------------------------------------------------------------------------
void AreaPicker::SetPickCoords(double x0, double y0, double x1, double y1)
{
  this->X0 = x0;
  this->Y0 = y0;
  this->X1 = x1;
...
int AreaPicker::AreaPick(double x0, double y0, double x1, double y1, vtkRenderer* renderer)
{
  this->Initialize();
  this->X0 = x0;
  this->Y0 = y0;
...
void AreaPicker::DefineFrustum(double x0, double y0, double x1, double y1, vtkRenderer* renderer)
{
  this->X0 = (x0 < x1) ? x0 : x1;
  this->Y0 = (y0 < y1) ? y0 : y1;
  this->X1 = (x0 > x1) ? x0 : x1;
...
int AreaPicker::PickProps(vtkRenderer* renderer)
{
    if (renderer == nullptr)
    {
        vtkErrorMacro(<< "Must specify renderer!");
    ...
    for (props->InitTraversal(pit); (prop = props->GetNextProp(pit));)
    {
        for (prop->InitPathTraversal(); (path = prop->GetNextPath());)
        {
            ...
            auto actor = vtkActor::SafeDownCast(propCandidate);
            //cout << "id of atom is : "<< actor->AtomId()+1<< endl;

            double dist;

            if (this->ABoxFrustumIsect(bounds, dist))
            {
                if (this->Prop3Ds->IsItemPresent(propCandidate))   continue;
                if (dist > 100000.0) continue;

                this->Prop3Ds->AddItem(static_cast<vtkProp3D*>(propCandidate));
                //cout << "id of atom is : "<< actor->AtomId()+1<<"   " <<dist<< endl;
...
/*
    this->Prop3Ds->InitTraversal();
    cout << "There are "<< NumSelectedProps()<< " is selected\n";
    for (unsigned int  i=0;i<this->Prop3Ds->GetNumberOfItems () ;i++)
    {
        auto actor = vtkActor::SafeDownCast(this->Prop3Ds->GetNextProp ());
        cout <<actor->GetProperty()->GetClassName()<<endl;
        cout << "id of atom is : "<< actor->AtomId()+1<<"   "<< actor->AtomId1()+1<<"   " << endl;
    }

*/
    return NumSelectedProps();


    if(NumSelectedProps()>1) return NumSelectedProps();
 ...
 unsigned int  AreaPicker::NumSelectedProps()
 {
     return Prop3Ds->GetNumberOfItems();
 }

//------------------------------------------------------------------------------
// converts the propCandidate into a vtkAbstractMapper3D
// and returns its pickability
int AreaPicker::TypeDecipher(vtkProp* propCandidate, vtkAbstractMapper3D** mapper)
{
    int pickable = 0;
    *mapper = nullptr;
...
int AreaPicker::ABoxFrustumIsect(double* bounds, double& mindist)
{
...
//--------------------------------------------------------------------------
void AreaPicker::PrintSelf(ostream& os, vtkIndent indent)
{
